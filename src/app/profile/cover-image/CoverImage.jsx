...
export default function CoverImage() {
  ...
  const originImg = useRef(user.cover) // url cover được lưu trong localStorage
  const defaultOffsetY = useRef(user.cover_offsetY)
  const defaultOffsetX = useRef(user.cover_offsetX)
  ...
  useEffect(() => {
    ...
    imgCom.onload = () => {
      ...
      if (imageRatio > 3) {
        ...
        console.log('Thong so moi', imgW, imgH, scaleRatio)
        isDragX.current = true
        isDragY.current = false

        // nếu đang upload ảnh cover mới từ file, tạo điểm offset cho ảnh hiển thị chính giữa container
        if (isPreviewCover) {
          defaultOffsetX.current = -(imgW - fullWidth) / 2
          defaultOffsetY.current = 0
          console.log(
            'defaul offset new cover',
            defaultOffsetX.current,
            defaultOffsetY.current
          )
        }
        startTransition(() => {
          setImageHeight(imgH)
          setImageWidth(imgW)
          setOffsetY(0)
          setOffsetX(scaleRatio * defaultOffsetX.current)
        })
        ...
        isDragY.current = true
        // nếu đang upload ảnh cover mới từ file, tạo điểm offset cho ảnh hiển thị chính giữa container
        if (isPreviewCover) {
          defaultOffsetX.current = 0
          defaultOffsetY.current = -(imgH - fullHeight) / 2
        }
        startTransition(() => {
          setOffsetX(0)
          setOffsetY(scaleRatio * defaultOffsetY.current)
          setImageHeight(imgH)
          setImageWidth(imgW)
        })
      } else {
    ...
    if (isPreviewCover) {
      if (isDragY.current) {
        ...
        setOffsetX((prev) => {
          ...
          console.log('Min OffsetX', minOffset)
          const newOffsetX = Math.max(Math.min(newOffset, maxOffset), minOffset)
          console.log('new OffsetX', newOffsetX)
          return newOffsetX
        })
      }
    }
  ...
  const setNewCoverImage = (urlImG) => {
    ...
    originImg.current = coverUrl
    setCoverUrl(urlImG)
  }

  function cancelEdit() {
    console.log('Cancel Edit:', originImg.current)
    scaleRatio = defaultScaleRatio
    isDragX.current = false
    isDragY.current = false
    startTransition(() => {
      console.log(originImg.current)
      setCoverUrl(originImg.current)
      setIsPreviewCover(false)
      setOffsetX(defaultOffsetX.current * scaleRatio)
      setOffsetY(defaultOffsetY.current * scaleRatio)
    })
  }
